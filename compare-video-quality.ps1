...
if (-not (Get-Command ffmpeg -ErrorAction SilentlyContinue)) {
    Write-Error "ffmpeg non trovato nel PATH. Installalo (https://www.gyan.dev/ffmpeg/builds/) e riprova."
    exit 1
}
if (-not (Get-Command ffprobe -ErrorAction SilentlyContinue)) {
    Write-Error "ffprobe non trovato nel PATH. Fa parte dello stesso pacchetto di ffmpeg."
    exit 1
}
if (-not (Test-Path -LiteralPath $DirA -PathType Container)) {
    Write-Error "'$DirA' non e' una cartella valida."
    exit 1
}
if (-not (Test-Path -LiteralPath $DirB -PathType Container)) {
    Write-Error "'$DirB' non e' una cartella valida."
    exit 1
}
$fullA = (Resolve-Path -LiteralPath $DirA).ProviderPath
$fullB = (Resolve-Path -LiteralPath $DirB).ProviderPath
if ($fullA -ieq $fullB) {
    Write-Error "DirA e DirB puntano alla stessa cartella."
    exit 1
}
...
function ConvertTo-SafeFileName {
    param([string]$Name)
    # elenco fisso dei caratteri non ammessi nei nomi file/cartella su Windows
    # (non si usa [IO.Path]::GetInvalidFileNameChars(): riflette l'OS di
    # esecuzione, non quello di destinazione, ed e' un set molto piu' permissivo
    # su Linux/macOS)
...
if (-not $commonKeys -or $commonKeys.Count -eq 0) {
    Write-Host "Nessun file con lo stesso nome trovato tra le due cartelle."
    Remove-Item -Recurse -Force -LiteralPath $WorkTmp -ErrorAction SilentlyContinue
    exit 0
}

Write-Host "Trovate $($commonKeys.Count) coppie di file con lo stesso nome. Avvio analisi..."
Write-Host ""
...
foreach ($key in $commonKeys) {
    ...
    if (-not $wA -or -not $hA -or -not $wB -or -not $hB) {
        Write-Host "  -> Skip: impossibile leggere lo stream video (file non valido/corrotto?)."
        Write-Host ""
        continue
    }
    ...
    if ($durA -le 0 -or $durB -le 0) {
        Write-Host "  -> Skip: durata non disponibile per uno dei due file."
        Write-Host ""
        continue
    }
    ...
    if ($nBlur -eq 0) {
        Write-Host "  -> Skip: impossibile estrarre fotogrammi campione da uno dei due file."
        Write-Host ""
        continue
    }
    ...
    Write-Host "  A: ${wA}x${hA} $codecA $brAk kbps  nitidezza(blur)=$(Fmt $blurAAvg '0.00')  punteggio=$(Fmt $scoreA '0.000')"
    Write-Host "  B: ${wB}x${hB} $codecB $brBk kbps  nitidezza(blur)=$(Fmt $blurBAvg '0.00')  punteggio=$(Fmt $scoreB '0.000')"
    Write-Host "  -> $verdict$durNote"
    Write-Host "  screenshot: $pairDir"
    Write-Host ""
...
Write-Host "Fatto. Report in: $ReportMd (e $ReportCsv)"
Write-Host "Comandi di spostamento suggeriti (commentati, da rivedere): $MoveScript"
